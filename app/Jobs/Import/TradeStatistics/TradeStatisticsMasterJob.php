...
class TradeStatisticsMasterJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        (new FastExcel)->sheet($this->sheetIndex)->withoutHeaders()->import($absolutePath, function ($row) use (&$chunk, &$rowIndex, $chunkSize) {
            ...
            if (count($chunk) === $chunkSize) {
                $this->batch()->add(new ProcessTradeStatisticsWorkerJob($this->importRecordId, $chunk, $this->mapping, $this->extraData));
                $chunk = [];
            }
        });
